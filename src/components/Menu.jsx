...
export default function Menu({
  ...
  // refèrences sur les inputs file 
  ...
  const importBacklog = async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(json)) throw new Error("Le backlog doit être une liste");
  ...
  const canStart = players.length > 0 && Array.isArray(backlog) && backlog.length > 0;
  return (
    <div className="menu-root">
      <div className="menu-card">
        ...
        <section className="menu-section">
          ...
          {players.length === 0 ? (
            <p className="hint">Ajoute au moins un joueur pour pouvoir lancer la partie.</p>
          ) : (
            <ul className="players-list">
              {players.map((p) => (
                <li key={p} className="player-item">
                  <span className="player-avatar">{p[0]?.toUpperCase() ?? "?"}</span>
        ...
        <section className="menu-section">
          ...
          <div className="buttons-grid">
            ...
            <div className="btn-block">
              ...
              <p className="hint small">
                Format attendu : liste d’US <code>[{"{ id, title, estimate? }"}]</code>
              </p>
            </div>
            {/* Export état */}
            <div className="btn-block">
              <button className="btn secondary" onClick={onExportState} type="button">
                Exporter l’état
              </button>
              <p className="hint small">Sauvegarde la configuration actuelle (JSON).</p>
            </div>
            {/* Import état */}
            <div className="btn-block">
              ...
              <input
                ...
                style={{ display: "none" }}
              />
              <p className="hint small">Permet de reprendre une partie sauvegardée.</p>
            </div>
          </div>
          {/* Résumé backlog */}
          <div className="backlog-summary">
            ...
            {Array.isArray(backlog) && backlog.length > 0 ? (
              <span>
                Backlog chargé : <b>{backlog.length}</b> élément{backlog.length > 1 ? "s" : ""}.
              </span>
            ) : (
        ...
        <div className="menu-footer">
          <button className="btn primary big" disabled={!canStart} onClick={onStart} type="button">
            Lancer la partie
          </button>
        </div>
      </div>
